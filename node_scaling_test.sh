...
cp ../node_scaling_test.sh used-script-copy.txt
...
for i in $(seq "$2" "$3" "$4"); do
	...
	echo "Running test $i.2 - All on..." | tee -a LOG.txt
	output2=$("./../$5" -Ihpx.stacks.use_guard_pages=0 "-t$i" $octotiger_args p2p_kernel_type=soa_cpu p2m_kernel_type=soa_cpu m2m_kernel_type=soa_cpu m2p_kernel_type=soa_cpu)
	check_results "$1" "$output2"
	# Clean up results
	clean_output_computational="$i,$(echo "$output1" | grep 'Computation' | sed 's/Computation: //g'),$(echo "$output2" | grep 'Computation' | sed 's/Computation: //g'))"
	clean_output_total="$i,$(echo "$output1" | grep 'Total' | sed 's/Total: //g'),$(echo "$output2" | grep 'Total' | sed 's/Total: //g')')"
